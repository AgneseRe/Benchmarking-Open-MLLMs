# QwenVL Evaluator for Odd-One-Out Anomaly Detection
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Evaluate QwenVL models')
    parser.add_argument('--model', type=str, default='Qwen2.5-VL-7B-Instruct', choices = ['Qwen2.5-VL-3B-Instruct', 'Qwen2.5-VL-7B-Instruct', 
                        'Qwen2.5-VL-32B-Instruct', 'Qwen3-VL-4B-Instruct', 'Qwen3-VL-4B-Thinking', 'Qwen3-VL-8B-Instruct',
			            'Qwen3-VL-8B-Thinking'], help='Model to use for evaluation')
...
# ===== QWENVL EVALUATOR =====
class QwenEvaluator(GenericEvaluator):

  def __init__(self, config, model_name: str = 'Qwen/Qwen2.5-VL-7B-Instruct'):
    ...
    if hasattr(self.processor, 'image_processor'):
        print(f"Image processor size: {self.processor.image_processor.size}")

  def predict(self, original_image_path: str, som_image_path: str, class_name: str, use_class_name: bool) -> Tuple[str, str, float]:
    """Query QwenVL. It returns the odd_index, output response and inference time."""
    try:
        ...
        inputs = self.processor(text=[text], images=image_inputs, videos=video_inputs, padding=True, return_tensors="pt").to(self.config.DEVICE)
        ...
        generation_config = GenerationConfig(
            do_sample=False,
            max_new_tokens = 2048 if is_thinking_model else (1024 if self.config.USE_COT else 64),
...
def main():
    ...
    try:
        qwen_evaluator = QwenEvaluator(config, f'Qwen/{args.model}')
    except Exception as e:
